...
class TestDataset:
    ...
    def ddp_setup(self, rank: int, world_size: int):
        ...
        self._tensors = self._tensors[:len(self._tensors) - (len(self._tensors) % (self.batch_size * world_size))]
        self.ddp = True

    def __iter__(self):
        for i in range(len(self)):
            if self.ddp:
                seqs = self._tensors[(i * self.world_size + self.rank) * self.batch_size:
                            (i * self.world_size + self.rank + 1) * self.batch_size, len(self._tensors)]
            else:
