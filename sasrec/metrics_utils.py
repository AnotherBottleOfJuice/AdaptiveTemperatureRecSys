...
def get_metrics(
    targets: torch.Tensor, candidates: torch.Tensor, topk: int
) -> Dict[str, float]:
    ...
    if relevant.any():
        ranks = (
            torch.nonzero(relevant, as_tuple=False).flatten().to(dtype=torch.float32)
        )
    ...
    ndcg_denominator = torch.sum(
        ...
        / torch.log2(
            torch.arange(
                1, recall_denominator + 1, device=candidates.device, dtype=torch.float32
            )
            + 1.0
        )
    )
    ...
    return {
        ...
        "ndcg": (ndcg_numerator / ndcg_denominator).item()
        if ndcg_denominator.item() > 0
        else 0.0,
    }
...
def evaluate(
    ...
    if covered_candidates:
        coverage = torch.unique(torch.cat(covered_candidates)).numel() / max(
            catalog_size, 1
        )
    else:
        coverage = 0.0
