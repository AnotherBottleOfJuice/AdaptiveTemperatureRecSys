...
class MACLossPerUserTau(Tau):
    ...
    def __call__(self, ctx: TauContext) -> Tensor:
        ...
        tau = (
            (logits - self.base_threshold) * self.linear_coeff + self.initial_tau
        ).clamp(self.tau_min, self.tau_max)
        return tau

    def experiment_name(self) -> str:
        return (
            f"MACLossPerUser[min={float(self.tau_min):.5g},"
            f"max={float(self.tau_max):.5g},"
            f"base={float(self.base_threshold):.5g},"
            f"coeff={float(self.linear_coeff):.5g}]"
        )
