...
def get_train_events(
    ...
    item_to_freq: pl.DataFrame | None,
    ...
    if item_to_freq is not None:
        train_events = train_events.join(
            item_to_token_id.join(item_to_freq, on="item_id", how="left").drop(
                "item_id"
            ),
            on="token_id",
            how="left",
    ...
    return train_events.group_by("uid", maintain_order=True).tail(
        max_events_per_user - 1
    )


def get_train_histories(train_events: pl.DataFrame, bos: int) -> pl.DataFrame:
    return (
        pl.concat(
            [
                (
